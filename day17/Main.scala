...
object Main extends App {
  ...
  type Map = mutable.Map[Pos, String]
  ...
  def parameterSum(map: Map, maxX: Int, maxY: Int): Int = {
    0.until(maxY).map { y =>
      0.until(maxX).map { x =>
        ...
        map(pos) match {
          case "#" => {
            if(List(northOf(pos), southOf(pos), westOf(pos), eastOf(pos))
              .filter { (p: Pos) => map.contains(p) && map(p) == "#" }.size == 4) {
              x * y
            } else {
              0
            }
  ...
  def draw(map: mutable.Map[Pos, String], maxX: Int, maxY: Int): Unit = {
    0.until(maxY).foreach { y =>
      0.until(maxX).foreach { x =>
        print(prettify(map(Pos(x, y))))
      }
      println()
    }
  }

  def prettify(k: String): String = {
    k match {
      case "#" => s"$WHITE█$RESET"
      case "." => s"$BLACK░$RESET"
      case "^" => "⬆"
  ...
  def parse(outputs: List[Int]): (mutable.Map[Pos, String], Int, Int) = {
    val map = mutable.Map[Pos, String]()
    var maxX = -1
    var x = 0
    var y = 0
    outputs.foreach { k =>
